Program avrgnorm
  ...
  real(kind=8),dimension(:),allocatable::pop1, pop2, popsum, popdiff, nrm, rlacf, imacf, abacf, ehr, time, rlex, imex, abex
  real(kind=8)::pop1av, pop2av, popsumav, popdiffav, nrmav, rlacfav, imacfav, abacfav, rlexav, imexav, abexav, ehrav, timeav
  ...
  do l=1,tot
     ...
     write (m,*) "Time Norm Re(ACF(t)) Im(ACF(t) |ACF(t)| Re(Extra) Im(Extra) |Extra| Sum(HEhr) Pop1 Pop2 Pop1+Pop2 Pop2-Pop1"
     write (m,*) ""
     write (m,*) ""
  ...
  do k = 1,lines(1)
     ...
     do i=1,tot
        n=1130+i
        read(n,*,iostat=ierr)time(i),nrm(i),rlacf(i),imacf(i),abacf(i),rlex(i),imex(i),abex(i),&
                                ehr(i),pop1(i),pop2(i),popsum(i),popdiff(i)
        if (time(i).ne.time(1)) then
           print "(a,e15.8,a,i0)", "File synchronisation error when reading at time ", time(1) ," in unit ", n
     ...
     do i=1,tot
        ...
        write (m,"(13(1x,e15.8))") timeav, nrmav, rlacfav, imacfav, abacfav, rlexav, imexav, abexav, &
                                        ehrav, pop1av, pop2av, popsumav, popdiffav
  ...
  do l=1,tot
     ...
     if (l==1) then     
        lngchar = 'plot "normpop_cumul_'//trim(repstr)//'.out" u 1:13 t "'//trim(repstr)//' Reps" w l'
     else
        lngchar2 = lngchar
        lngchar = trim(lngchar2)//', "normpop_cumul_'//trim(repstr)//'.out" u 1:13 t "'//trim(repstr)//' Reps" w l'
     end if

  end do

  lngchar2 = lngchar
  lngchar = trim(lngchar2)//', "normpop_cumul_'//trim(repstr)//'.out" u 1:2 t "Total Av Norm" w l'
  ...
  if (tot .gt. 1) then
     ...
     do l=1,tot
        ...
        if (l==1) then     
           lngchar = 'plot "popdiffresiduals.out" u 1:2 t "'//trim(repstr)//' Reps" w l'
        else
           lngchar2 = lngchar
           lngchar = trim(lngchar2)//', "" u 1:'//trim(lstr)//' t "'//trim(repstr)//' Reps" w l'
        end if
     ...
     write(176,"(a)") 'set title "Graph of residuals with different numbers of repetitions"'

...
!*      8) Generating the basis function single configurational d prefactors for electronic states
!*      
!*************************************************************************************************!
...
!*************************************************************************************************!
  ...
  subroutine genbasis(bs, mup, muq,alcmprss, gridsp, t, initgrid)   !   Level 1 Subroutine
...
!--------------------------------------------------------------------------------------------------
  ...
  subroutine gen_swtrn(bs,mup,muq,alcmprss,t)
    ...
    integer::m, k, j, x, n, ierr, trtype, swrmsize, restart, kcut, kstrt, steps, redo, def_stp2, stepback, genflg
    ...
    if (mod(in_nbf,def_stp2)==0) then
       ...
       print '(a,i0,a,i0)', "In_nbf is not an integer multiple of ", def_stp2, " or ", def_stp2 - 1, &
                                    ", which are the default number of steps."
       print '(a)', "To have a different number of steps in a swarm train or train swarm, modify value of def_stp in gen_swtrn sub" 
       errorflag = 1
       return
    end if
    ...
    if (trtype == 1) then
       ...
       print '(a,i0)', "Error! Train swarm/swarm train identifier is wrong. Should be 1 or 2 but got ", trtype
       errorflag = 1
       return
...
!--------------------------------------------------------------------------------------------------
  ...
  subroutine gen_grid(bs,mup,muq,initgrid,gridsp,t)
    ...
    if (size(initgrid,1).ne.in_nbf) then
       print *, "Error in generating the initial grid overlap matrix. Sizes are not equal to in_nbf"
  ...
  subroutine gen_grswm(bs,mup,muq,alcmprss,initgrid,gridsp,t)
    ...
    do p=1,psizez
       do q=1,qsizez
          ...
          bf=bs(k)
          do
             bf%z(1)=cmplx((qstrt+(gridsp*(q-1)*sigq)),(pstrt+(gridsp*(p-1)*sigp)),kind=8)
             do m=2,ndim
                bf%z(m)=cmplx((ZBQLNOR(muq(m),sigq*sqrt(alcmprss))),&
                ((1.0d0/hbar)*(ZBQLNOR(mup(m),sigp*sqrt(alcmprss)))),kind=8)
             end do
  ...
  subroutine genD_big(bs, mup, muq, restart)   !   Level 1 Subroutine
    ...
    if (((basis=="GRID").or.(basis=="GRSWM")).and.(mod(in_nbf,2)==1)) then
       ...
       do k=1,size(bs)
          ...
          C_k(k) = ovrlpij(zpq, zinit) * dconjg(bs(k)%d_pes(in_pes)) * cdexp (-1.0d0*i*bs(k)%s_pes(in_pes)) !added sc amplitudes 27/01/15
       end do
       ...
       ovrlp_mat=ovrlpphimat(bs)   ! changed to phi matrix for trains 27/01/2015

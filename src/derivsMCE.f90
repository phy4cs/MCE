...
!*
!*      1) Calling the time derivative subroutines for each variable and returning the results
!*        to the timestep subroutine
!*      2) Calculating the time derivative of the z values
!*      3) Calculating the time derivative of the single configuration d prefactor for MCEv1
!*      4) Calculating the time derivative of the single configuration d prefactor for MCEv2
!*      5) Calculating the time derivative of the classical action
!*      6) Calculating the time derivative of the multi-configuration D prefactor for MCEv1
!*      7) Calculating the time derivative of the multi-configuration D prefactor for MCEv2
!*      8) Calculating the first component of the Hamiltonian average used in 7)
!*      9) Calculating the second component of the Hamiltonian average used in 7)
!*     10) Calculating the third comnponent of the Hamiltonian average used in 7) and also in 3)
!*      
!*************************************************************************************************!
...
!*************************************************************************************************!
  ...
  subroutine deriv(bsin, bsout, x, time, genflg)
    ...
    allocate (dz(size(bsin),ndim),dd(size(bsin),npes),ds(size(bsin),npes),dD_big(size(bsin)), stat=ierr)
    if (ierr/=0) then
...
!--------------------------------------------------------------------------------------------------
  ...
  function zdot(bsin,t)
    ...
    allocate(zdottemp(ndim), z(ndim), dhdz(npes,npes,ndim), a(npes), ac(npes), stat = ierr)
    if (ierr/=0) then
...
!--------------------------------------------------------------------------------------------------
  ...
  function ddotv1(bsin,time)
    ...
    complex(kind=8), dimension (:,:),allocatable :: ovrlp, ovrlpin, d2H_temp, zczd, ddot_temppes
    ...
    allocate(ovrlp(nbf,nbf),ovrlpin(nbf,nbf),zczd(nbf,nbf),d2H_temp(nbf,nbf),d2H(nbf,nbf,npes),atemp(nbf), &
              ddot_temp(nbf),ddot_out(nbf),ddot_in(nbf),ddot_temppes(nbf, npes),stat = ierr)
    if (ierr/=0) then
    ...
    do r=1,npes   
       ...
       do k=1,nbf
          ...
          do j=1,nbf
             ovrlpin(j,k) = ovrlp(j,k)    !!!! ovrlp is changed by the matinv subroutine, so must be redefined for each pes
          end do
       end do
    ...
    deallocate(ovrlp,ovrlpin,zczd,d2H_temp,d2H,atemp,ddot_temp,ddot_out,ddot_in,ddot_temppes,stat = ierr)
    if (ierr/=0) then
  ...
  function ddotv2(bsin,t)
    ...
    do k=1,size(bsin)
       ...
       do r=1,npes
          do s=1,npes
             if (r.ne.s) then
                ddotv2(k,r) = ddotv2(k,r) + Hkk(r,s)*ovrlpij(z,z)*dk(s)*cdexp(i*(Sk(s)-Sk(r)))
             end if
          end do
       end do
    end do
...
!--------------------------------------------------------------------------------------------------
  ...
  function bigDdotv2(bsin,x, time)
    ...
    complex(kind=8), dimension (:,:), allocatable :: h_av_jj, h_av_jk, zconjzdot, d2H, d2H2, d2Hdiff
    ...
    allocate(ovrlp(nbf,nbf),ovrlpphi(nbf,nbf),h_av_jj(nbf,nbf),h_av_jk(nbf,nbf),zconjzdot(nbf,nbf),&
             d2H(nbf,nbf),d2H2(nbf,nbf),d2Hdiff(nbf,nbf),tempD(nbf),chk(nbf),Ddot(nbf),d2HD(nbf),Dtemp(nbf),stat = ierr)
    if (ierr/=0) then
    ...
    if ((x.eq.1).and.(time.eq.0.0d0)) then
       do j=1,nbf
          do k=1,nbf
             if (((ovrlpphi(j,k) - ovrlp(j,k)).ne.0.0d0).and.(basis.ne."TRAIN").and.(basis.ne."SWTRN")) then
                print '(a)', "Error! Initial phi-overlap matrix has disimilarilies to z-overlap matrix"
                print '(a,i0,a,i0,a)', "These matricies should be identical but differences found at coordinate ", j,",",k,"."
                print '(a,4(e15.8,a))', "Expected (",dimag(i*ovrlp(j,k)),",",dimag(ovrlp(j,k)),") but got (",&
   ...
   call deallocham(H)
    ...
    deallocate(ovrlp,ovrlpphi,h_av_jj,h_av_jk,zconjzdot,d2H,d2H2,d2Hdiff,tempD,chk,Ddot,d2HD,Dtemp,stat = ierr)
    if (ierr/=0) then

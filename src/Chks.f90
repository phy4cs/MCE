...
!*
!*      1) Checking the initial norm and population sum falls in acceptable range, and if desired
!*        changes the compression parameter and restarts the basis set generation.
!*      2) Checking the position components of the coherent state are not too widely spaced
!*      3) Checking that the conserved quantites do not deviate too much from their initial values
!*      
!*************************************************************************************************!
...
!*************************************************************************************************!
  ...
  subroutine initnormchk(bs,recalcs,restart, alcmprss, gridsp, absnorm, popsum)   !   Level 1 Subroutine
    ...
    if(size(bs).ne.1) then
       ...
       if (cmprss.eq."N") then      
          if ((absnorm.gt.uplimnorm).or.(absnorm.lt.lowlimnorm)) then
             print "(a,e13.5e3)", "Warning. Initial Norm outside established parameters, with a value of ", absnorm
             print *, ""
  ...
  subroutine enchk(bf,t,n,redo,k)
    ...
    if (ECheck.eq."YES") then
       ...
       if ((Echk.gt.Ebfmax).or.(Echk.lt.Ebfmin)) then
          if (n.lt.Ntries) then
             n = n+1
             print *,"Basis ", k, " did not meet energy requirements. Recalculating..."
             redo=1
          else
             print *,"Basis ", k, " recalculated ", n, "times but still outside acceptable region."
...
!--------------------------------------------------------------------------------------------------
  ...
  subroutine trajchk(bs)   !   Level 1 Subroutine
    ...
    do j = 1,size(bs)
       ...
       do m=1,ndim
          ...
          if (abs(trajq).gt.20000) then
             print '(a,i0,a,i0,a,e16.8)', "Trajectory ", j, " in dof ", m, " is equal to ", dble(z(m))*dsqrt(2.0d0)
             flag = flag + 1
          end if
       end do
    end do
    ...
    if (flag.gt.0) then
       errorflag = 1
       print "(1x,i0,a)", flag, " trajectories have position components outside acceptable range."
       return
    end if

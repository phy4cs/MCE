...
!--------------------------------------------------------------------------------------------------
  ...
  subroutine Hij_iv(H,z1,z2,t)
    ...
    do m=1,ndim
       Htemp(m) = (0.0d0,0.0d0)
       Htemp(m) = Htemp(m) - ((hbar**2)*gam/(4.0d0*mass_iv))*(z1c(m)**2.0d0+z2(m)**2.0d0-2.0d0*z1c(m)*z2(m)-1.0d0)
...
!--------------------------------------------------------------------------------------------------
  ...
  function dh_dz_iv(z,t)
    ...
    do m=1,ndim    
       ...
       dhdztmp = dhdztmp + sqrt(2.0*(eta**3.0)*gam/lambda_iv)*rho(m)*exp(-1.0d0*eta*rho(m)**2.0)
       ...
       do j=1,size(bs)
          ...
          do m=1,ndim
             dipole_iv = dipole_iv + (-1.0d0*Dc(j)*D(k)*cdexp(i*(s(k)-s(j)))* ovrlp*&
                2.0d0*sqrt((eta**3.0d0)/lambda_iv)*rho(m)*cdexp(-1.0d0*eta*(rho(m)**2.0d0)))
          end do
       end do
    end do

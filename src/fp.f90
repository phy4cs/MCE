...
!--------------------------------------------------------------------------------------------------
  ...
  subroutine readparams_fp
    ...
    if (n.ne.3) then
       print *, "Not all required variables read in readparams_fp subroutine"
       errorflag = 1
       return
    end if
...
!--------------------------------------------------------------------------------------------------
  ...
  subroutine Hij_fp(H,z1,z2)
    ...
    H(1,1) = (-1.0d0/(4.0d0*gam*mass_fp*hbar))*sum(dconjg(z1(1:ndim))**2+z2(1:ndim)**2-2*dconjg(z1(1:ndim))*z2(1:ndim)-1)

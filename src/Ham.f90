...
!*      5) Calculating the sum of the norms of the individual configurations E_j<phi_j|phi_j>
...
!*     10) Linear equation calculation for use in calculating intial D or time derivative.
!*     11) Matrix Inversion subroutine to be used as an alternative to 10) for initial D
!*         but increases runtime by ~3000% so not recommended for use. Included by matfun flag.
!*     12) Matrix inversion subroutine very similar to 11), to be used for time propagation.
!*      
!*************************************************************************************************!
    ...
    do k=1,n
       do j=1,k-1
          ...
          if (diff.gt.1.0d-6) then
             ...
             nonhermcount = nonhermcount + 1
          end if
       end do
    end do

    if (nonhermcount.ne.0) then
       print "(a,i5,a,i5,a,i5)", "Error! Overlap matrix is Non-Hermitian. ", 2*nonhermcount, &
...
!--------------------------------------------------------------------------------------------------
  ...
  function ovrlpij(z1,z2)
    ...
    if (abs(ovrlpchk-dconjg(ovrlpij)).gt.1.0d-10) then
       print*, "Error in ovrlpij function. Reversing the arrays does not conjugate them"
       print "(2(a,2(es20.12e3)))", "Ovrlpij value is ", ovrlpij, " while ovrlpchk value is ", ovrlpchk
...
!--------------------------------------------------------------------------------------------------
  ...
  function ovrlpphimat(bs)
    ...
    real(kind=8) :: diff, diffsum
    integer::k,j,r,nonhermcount, n, m
   ...
   nonhermcount = 0
    ...
    if (nonhermcount.ne.0) then
       print "(a,i5,a,i5,a,i5)", "Error! Phi overlap matrix is Non-Hermitian. ", 2*nonhermcount, &
                     " unmatched off-diagonal elements found in a ", n,"x",n," matrix"
       print "(a,es16.8e3)", "Diffsum = ", diffsum
       errorflag = 1
       return
...
!*************************************************************************************************!
  ...
  function norm(bs)   !   Level 1 Function
    ...
    if (absnorm.gt.1.2d0) then
       print "(a)", "Norm is greater than 1.2. Simulation has failed and trajectories are likely to explode"
    ...
    do k=1,size(bs)
       do j=1,size(bs)
          ...
          cpop = cpop + dconjg(bs(j)%D_big) * ovrlpij(zj,zk) * bs(k)%D_big &
                   * dconjg(bs(j)%d_pes(pes)) * bs(k)%d_pes(pes) * cdexp(i*(bs(k)%s_pes(pes)-bs(j)%s_pes(pes)))
       end do
    end do
...
!    do k=1,size(bs)
!       print *, "k = ", k, " a = ", bs(k)%a_pes(1), bs(k)%a_pes(2)
!    end do

    absnorm=sqrt(dble(innorm*dconjg(innorm)))

    if ((absnorm.gt.1.2d0).and.(cmprss=="N")) then
       print *, "Norm is greater than 1.2. Simulation has failed and trajectories are likely to explode"
       errorflag = 1
    end if
...
!--------------------------------------------------------------------------------------------------
  ...
  function acf(bs, mup, muq)   !   Level 1 Function
    ...
    allocate(zinit(ndim), Dbig(size(bs)), d(size(bs)), s(size(bs)), ovrlp(size(bs)), stat = ierr)
    if (ierr/=0) then
    ...
    do k=1,size(bs)
       ...
       do m=1,ndim
          ...
          if (chk.gt.20000d0) then
             ...
             ovrlp(k,m) = cdexp((dconjg(zinit(m))*bs(k)%z(m))-(0.5d0*dconjg(zinit(m))*zinit(m))&
                    -(0.5d0*dconjg(bs(k)%z(m))*bs(k)%z(m)))
...
!--------------------------------------------------------------------------------------------------
  ...
  subroutine matinv(ovrlp, C_k, D, restart)   !   Level 2 Subroutine
    ...
    integer::l, j, k, n, info, LWORK, nonhermcount, ierr
    ...
    if (nonhermcount.ne.0) then
       print "(a,i5,a,i5,a,i5)", "Error! Inverse matrix is Non-Hermitian. ", 2*nonhermcount, &
                     " unmatched off-diagonal elements found in a ", n,"x",n," matrix"
       restart = 1
       return
    end if

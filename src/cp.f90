...
!*
!*      IT SHOULD BE NOTED CURRENTLY THIS ONLY USES THE 3D GAUSSIAN SYSTEM!
!*      
!*************************************************************************************************!
...
!--------------------------------------------------------------------------------------------------
  ...
  subroutine genzinit_cp(mup, muq)   !   Level 1 Subroutine
...
!--------------------------------------------------------------------------------------------------
  ...
  subroutine Hij_cp(H,z1,z2,t)
    ...
    do m=1,ndim
       z1c = dconjg(z1(m))
       Htemp(m) = (0.0d0,0.0d0)
       Htemp(m) = Htemp(m) - (1.0d0/4.0d0)*(z1c**2.0d0+z2(m)**2.0d0-2.0d0*z1c*z2(m)-1.0d0) !free particle
       if (abs(arho) < tiny(1.0d0)) then             !checks that not near singularity
          ...
          Htemp(m) = Htemp(m) + (c_error_func(sqrt(gam)*arho)/(ndim*arho))      !atomic potential, averaged over ndims
       end if
...
!--------------------------------------------------------------------------------------------------
  ...
  function dh_dz_cp(z,t) ! This is laid out so as to allow easy conversion to calculating off diagonal elements
    ...
    datpotdz = (2.0d0*sqrt(gam)*cdexp(-gam*rho2))/(sqrtpi*rho2)-(c_error_func(sqrt(gam)*arho)/(arho**3.0))
...
!-------------------------------------------------------------------------------------------------
    ...
    if (ierr/=0) then
       print *, "Error allocating basis set variable arrays for dipole acceleration calculation"
       errorflag = 1
       return
    end if
    ...
    do k=1,size(bs)
       do j=1,size(bs)
          ...
          arho = sqrt(rho2)
          datpotdz = (2.0d0*sqrt(gam)*cdexp(-gam*rho2))/(sqrtpi*rho2)-(c_error_func(sqrt(gam)*arho)/(arho**3.0))
          ovrlp = product(cdexp((dconjg(zj(1:ndim))*zk(1:ndim))-(0.5d0*dconjg(zj(1:ndim))*zj(1:ndim))&
    ...
    if (ierr/=0) then
       print *, "Error deallocating basis set variable arrays for dipole acceleration calculation"   
       errorflag = 1   
       return
    end if

...
!--------------------------------------------------------------------------------------------------
  ...
  subroutine Hij_hh(H,z1,z2)
    ...
    do m=1,(ndim-1)
       Htemp(m) = Htemp(m) + prefac*((z1c(m)**2.0)+(z2(m)**2.0)+(2.0*z1c(m)*z2(m))+1)*(z1c(m+1)+z2(m+1))
       Htemp(m) = Htemp(m) - prefac*((1.0/3.0)*((z1c(m+1)**3.0)+(z2(m+1)**3.0))+(3.0*(z1c(m+1)**2.0)*z2(m+1)))
...
!--------------------------------------------------------------------------------------------------
  ...
  function dh_dz_hh(z)
    ...
    dh_dz_hh (1,1,m) = z(m) + fact*((zc(m-1)**2.0)+(z(m-1)**2.0)+(2.0*zc(m-1)*z(m-1))&
                                    -(zc(m)**2.0)-(z(m)**2.0)-(2.0*zc(m)*z(m)))

    if (ndim.gt.2) then
       do m=2,ndim-1     
          dh_dz_hh (1,1,m) = z(m) + fact*((zc(m-1)**2.0)+(z(m-1)**2.0)+(2.0*zc(m-1)*z(m-1)) &
                 - (zc(m)**2.0)-(z(m)**2.0)-(2.0*zc(m)*z(m)) + (2.0*(zc(m)+z(m))*(zc(m+1)+z(m+1))))       
       end do
    end if
